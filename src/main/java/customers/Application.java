...
public class Application {
	...
	public static void main(String[] args) {
		...
		Comparator<Customer> ageComparator = (a,b)-> a.getAge()-b.getAge();
		Iterator<Customer> ageIterator = customerCollection.ageIterator(ageComparator);

		while(ageIterator.hasNext()){
			System.out.println(ageIterator.next());
		}

		System.out.println("********************Address Filter City********************");
		Predicate<Customer> chicagoPredicate = c -> c.getAddress().getCity().equals("Chicago");
		Iterator<Customer> addressFilter = customerCollection.filterIterator(chicagoPredicate);

		while(addressFilter.hasNext()){
			System.out.println(addressFilter.next());
		}

		System.out.println("********************Address Filter Zip********************");
		Predicate<Customer> zipPredicate = c -> c.getAddress().getZip().startsWith("12");
		addressFilter = customerCollection.filterIterator(zipPredicate);
		...
		Iterator<Customer> oddFilter = customerCollection.oddIterator(1);
